...
import re
import string
from lib.odict import odict as OrderedDict
...
class FTClient():
  ...
  def query(self, query, request_type=None):
    ...
    if lowercase_query.startswith("select") or \
      ...
      data = self._get(urllib.urlencode({'sql': query}))
      PATTERN =  re.compile(r'''((?:[^\n"']|"[^"]*"|'[^']*')+)''')

      for idx, line in enumerate(PATTERN.split(data)[1::2]): #enumerate(string.split(self._get(urllib.urlencode({'sql': query})), '\n')):
         if line.strip():
